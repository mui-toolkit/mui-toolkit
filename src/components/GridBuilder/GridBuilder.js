...
export default function GridBuilder() {
		...
		if (e.target !== dragNode.current) {
			...
			setList((oldList) => {
				...
				if (newList[1].items.length === 0) {
					setLetter(letter + 1);
					newList[1].items = [ { id: String.fromCharCode(letter), cols: 12, color: '#be00f8' } ];
				}
				return newList;
			});
		}
	...
	const handleDelete = (e, params) => {
		...
		setList((prevList) => {
			...
			console.log(newer);
			return [ { cols: 9, paper: classes.paperGrid, items: [ ...newer ] }, { ...prevList[1] } ];
		});
	};
	...
	const gridCodeBuilder = () => {
		...
		let gridItemMap = items.map((item) => `<Grid item xs={${item.cols}} > ${item.id} </Grid>`);
	...
	return (
		<React.Fragment>
			<Grid container direction="row" className={classes.container} justify="flex-end">
				{list.map((group, grpI) => (
					<Grid
						item
						xs={group.cols}
						onDragEnter={
							dragging && !group.items.length ? (e) => handleDragEnter(e, { grpI, itemI: 0 }) : null
						}
					>
							...
							>
								{group.items.map((item, itemI) => (
									<Grid
										...
										className={dragging ? getStyles({ grpI, itemI }) : classes.box}
										xs={item.cols}
										align="center"
										draggable
										onDragEnter={
